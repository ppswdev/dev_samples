...
class ViewController: UIViewController {
    ...
    @objc private func saveTokenTapped() {
        let token = "token_demo2_\(Date().timeIntervalSince1970)"
    ...
    @objc private func saveAPIKeyTapped() {
        let apiKey = "api_key_demo2_\(Date().timeIntervalSince1970)"
    ...
    @objc private func checkSharedDataTapped() {
        // 检查是否有来自Demo1的共享数据
        let sharedKey = "demo1_shared_token"
        if let sharedToken = KeychainManager.shared.read(key: sharedKey) {
            log("📱 检测到来自Demo1的共享Token: \(sharedToken)")
        } else {
            log("📱 未找到来自Demo1的共享数据。请先在Demo1中保存数据。")
        }
        
        // 为Demo1保存一个共享token
        let sharedToken = "Demo2_Shared_\(Date().timeIntervalSince1970)"
        KeychainManager.shared.save(key: "demo2_shared_token", value: sharedToken)
        log("📤 已为Demo1共享Token: \(sharedToken)")
    }
