...
class MyViewController: UIViewController, StoreKitDelegate {
    ...
    private func handleSubscriptionCancelled(_ productId: String, isFreeTrialCancelled: Bool) {
    ...
    override func viewDidLoad() {
        ...
        StoreKit2Manager.shared.onStateChanged = { [weak self] state in
            ...
            case .subscriptionCancelled(let productId, let isFreeTrialCancelled):
                self?.handleSubscriptionCancelled(productId, isFreeTrialCancelled: isFreeTrialCancelled)
...
**免费试用期取消检测：**
- `.subscriptionCancelled(productId, isFreeTrialCancelled: true)`：用户在免费试用期内取消
  - 可以引导用户重新订阅
  - 可以统计免费试用取消率
  - 可以优化免费试用体验
- `.subscriptionCancelled(productId, isFreeTrialCancelled: false)`：用户在付费订阅期内取消
  - 显示订阅将在当前周期结束时过期
  - 可以提供续订优惠或重新订阅引导
  - 可以统计付费订阅取消率
...
class SubscriptionManager: NSObject, StoreKitDelegate {
    ...
    func storeKit(_ manager: StoreKit2Manager, didUpdateState state: StoreKitState) {
        ...
        case .subscriptionCancelled(let productId, let isFreeTrialCancelled):
            handleSubscriptionCancelled(productId, isFreeTrialCancelled: isFreeTrialCancelled)
               ...
               let expirationDate = renewalInfo.expirationDate {
                ...
                if daysRemaining > 0 {
                    if isFreeTrialCancelled {
                        ...
                        showAlert(
                            title: "免费试用已取消",
                            message: "您的免费试用将在 \(daysRemaining) 天后结束。在此之前，您仍可继续使用所有功能。"
                        )
                        // 可以引导用户重新订阅
                    } else {
                        // 付费订阅期取消
